...
/*   Updated: 2025/11/24 13:31:38 by memillet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**ft_convert_in_list(int argc, char **argv)
{
	...
	i = 1;
	if (argc < 2)
		return (ft_printf ("Error : Invalid number of argument\n"), NULL);
	lst = malloc(sizeof (char *) * (ft_count_words(argc, argv) + 1));
	if (!lst)
		return (NULL);
	while (i < argc)
	{
...
int	ft_check_duplicate(char **lst)
{
	...
	i = 0;
	if (!lst)
		return (ft_printf("Error : Empty string\n"), 0);
	while (lst[j])
	{
		i = 0;
		while (i < j)
		{
			if (ft_strcmp(lst[i], lst[j]) == 0)
				return (ft_printf("Error : There is a duplicate int\n"), 0);
			i++;
		}
		j++;
	}
